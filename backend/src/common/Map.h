#pragma once

#include <vector>
#include <list>
#include <unordered_map>
#include <functional>

#include "../POIs/POI.h"
#include "./Point.h"
#include "./PointHash.h"
#include "../robots/Robot.h"

class Map {
private:
    std::vector<std::vector<unsigned char>> map;
    std::unordered_map<int, std::unique_ptr<POI>> pointsOfInterest;
    std::unordered_map<int, std::reference_wrapper<Robot>> robots;
    std::unordered_map<std::pair<Point, Point>, int, PointPairHash> distanceMap;

    static Route aStar(const std::vector<std::vector<unsigned char>>& map, Point ini, Point fin);

public:
    Map(std::vector<std::vector<unsigned char>> map) : map(std::move(map)) {}
    
    const std::vector<std::vector<unsigned char>>& getMap() const;
    const std::unordered_map<int, std::unique_ptr<POI>>& getPOIs() const;
    const std::unordered_map<int, std::reference_wrapper<Robot>>& getRobots() const;
    Robot& getRobot(int robotID);

    int getDistance(Point a, Point b);

    void addRobot(Robot& r);
    void deleteRobot(int robotID);
    void addPoi(std::unique_ptr<POI> poi);
    void deletePoi(Point position);

    void createForbidenZone(Point tl, Point br);
    void createSlowZone(Point tl, Point br);
    void createCommonZone(Point tl, Point br);

    Route startTask(Point start, Point end, Point robotPos) const;
};
